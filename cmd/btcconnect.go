...
import (
	...
	"github.com/hyperledger/firefly-btcconnect/internal/bitcoin"
	"github.com/hyperledger/firefly-btcconnect/internal/ffconnector"
	"github.com/hyperledger/firefly-btcconnect/internal/ffcserver"
...
var rootCmd = &cobra.Command{
	Use:   "btcconnect",
	Short: "Hyperledger FireFly Connector for the Bitcoin blockchain",
	Long:  ``,
...
func initConfig() {
	...
	bitcoinConf := connectors.SubSection(ConfigConnectorBitcoin)
	bitcoin.InitConfig(bitcoinConf)
}

func run() error {

	initConfig()
	err := config.ReadConfig("btcconnect", cfgFile)
	...
	ctx = log.WithLogger(ctx, logrus.WithField("prefix", "btcconnect"))
	...
	for i := 0; i < numConnectors; i++ {
		...
		case ConfigConnectorBitcoin:
			c = bitcoin.NewBitcoinConnector(childConnectorConf)
		default:
